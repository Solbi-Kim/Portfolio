...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting && e.intersectionRatio > 0.6) {
        index = Math.max(0, sections.indexOf(e.target));
      }
    });
  ...
  function scrollToIndex(next){
    ...
    const end   = sections[next].offsetTop;
    ...
    function frame(now){
      ...
      else { index = next; animating = false; }
    }
    requestAnimationFrame(frame);
  }
  ...
  scroller.addEventListener('wheel', (e) => {
    // 섹션 내부에 수직 스크롤 콘텐츠가 없다면 passive:false로 막아도 됨
    if (animating) { e.preventDefault(); return; }
    const dir = Math.sign(e.deltaY);
    if (!dir) return;
    // 스크롤 막대가 맨위/맨아래에서만 페이지 전환하고 중간에선 기본 스크롤 허용하고 싶으면 아래 조건 추가
    e.preventDefault();
    scrollToIndex(index + dir);
  }, { passive: false });

  // 키보드 지원(↓/PgDn/Space: 다음, ↑/PgUp/Shift+Space: 이전)
  window.addEventListener('keydown', (e) => {
    if (animating) return;
    const nextKeys = ['ArrowDown','PageDown',' '];
    const prevKeys = ['ArrowUp','PageUp'];
    if (nextKeys.includes(e.key) || (e.key === ' ' && !e.shiftKey)) {
      e.preventDefault(); scrollToIndex(index + 1);
  ...
  scroller.addEventListener('touchstart', (e)=>{ startY = e.touches[0].clientY; moved = 0; }, {passive:true});
  scroller.addEventListener('touchmove',  (e)=>{ moved = e.touches[0].clientY - startY; }, {passive:true});
  scroller.addEventListener('touchend',   ()=> {
    if (animating) return;
    const TH = 50; // 스와이프 임계
    if (moved < -TH) scrollToIndex(index + 1);
    else if (moved > TH) scrollToIndex(index - 1);
  });
})();
